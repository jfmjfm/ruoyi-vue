              ...
              <el-form :model="comparisonForm" label-width="100px">
                <el-form-item label="选择案例">
                  <el-checkbox-group v-model="comparisonForm.selectedCases">
                    <el-checkbox 
                      v-for="menuItem in menuItemsForComparison" 
                      :key="menuItem.caseId" 
                      :label="menuItem.caseId">
                      {{ menuItem.fullName }}
                    </el-checkbox>
                </el-checkbox-group>
                </el-form-item>
...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    const regionServiceId = to.query.region_service_id || to.query.regionServiceId;
    if (regionServiceId) {
      this.refreshData(regionServiceId, true);
    }
    next();
  },
  ...
  methods: {
    ...
    fetchServiceCases(regionServiceId) {
      ...
      return listProject_service_case_by_region_service_id(regionServiceId, params).then(response => {
        if (response && response.code === 200 && response.rows) {
          // 筛选匹配记录
    ...
    generateMenuItemsFromCases() {
      ...
      // 筛选标量优化的记录
      const optimizationCases = this.serviceCases.filter(caseItem => 
        caseItem && caseItem.description === '标量优化'
      );
      
      // 找到第一个baseline项目
      for (const caseItem of this.serviceCases) {
        if (caseItem && caseItem.caseName && caseItem.caseName.includes('baseline')) {
          baselineItem = caseItem;
          break;
        }
      }
      
      // 如果找到baseline项目，添加到菜单
      if (baselineItem) {
        // 分割案例名称，提取第二个'-'后的值
        const nameParts = baselineItem.caseName.split('-');
        let nameValue = '';
        ...
        if (nameParts.length >= 3) {
          ...
          // 没有'-'，使用完整名称
          nameValue = baselineItem.caseName;
        }
        ...
        const menuItem = {
          ...
          nameBottom: nameBottom,
          fullName: nameValue,
          icon: this.getCaseIcon(0),
          type: '0',
          caseId: baselineItem.id,
          caseData: baselineItem
        };
      ...
      // 添加标量优化的记录
      optimizationCases.forEach((caseItem, index) => {
        ...
        if (nameValue.length <= 2) {
          ...
          // 名称较长时，尝试按照常见分隔符拆分
          if (nameValue.includes('优化')) {
            const parts = nameValue.split('优化');
            nameTop = (parts[0] || '') + '优化';
            nameBottom = parts.length > 1 ? parts[1] : '';
          } else {
            // 默认均分处理
      ...
      // 更新对比分析用的菜单项列表，只包含非对比分析的菜单项
      this.menuItemsForComparison = items.filter(item => !item.isComparison && item.caseId);
      ...
      try {
        ...
        this.$nextTick(() => {
          ...
          this.comparisonForm.selectedCases.forEach(caseId => {
            // 从菜单项中查找对应的案例，而不是从所有服务案例中查找
            const menuItem = this.menuItemsForComparison.find(item => item.caseId === caseId);
            if (menuItem && menuItem.caseData) {
              selectedCaseData.push(menuItem.caseData);
              
              // 生成数据 - 基准情景与优化情景有不同的数据模式
              let data;
              if (menuItem.caseData.caseName && menuItem.caseData.caseName.includes('baseline')) {
                // 基准情景数据
                data = this.comparisonForm.timeScale === 'month' 
                  ? [65, 70, 76, 82, 90, 100, 95, 88, 80, 75, 70, 66]
                  : [72, 78, 85, 90, 95];
              } else {
                // 优化情景数据 - 比基准情景略高
                data = this.comparisonForm.timeScale === 'month' 
                  ? [70, 76, 82, 88, 95, 105, 100, 92, 85, 80, 75, 71]
                  : [78, 84, 90, 96, 102];
              }
              
              // 根据对比维度调整数据
              if (this.comparisonForm.dimension === 'actual') {
                // 实际利用维度的数据比潜在供给略低
                data = data.map(val => Math.round(val * 0.8));
              } else if (this.comparisonForm.dimension === 'balance') {
                // 供需平衡维度的数据为正负值
                data = data.map(val => Math.round(val * 0.6 - 40));
              }
              
              // 设置线条样式 - 基准情景使用虚线，优化情景使用实线
              const lineStyle = menuItem.caseData.caseName && menuItem.caseData.caseName.includes('baseline')
                ? { type: 'dashed', width: 2 }
                : { width: 2 };
          ...
          const option = {
            ...
            legend: {
              data: selectedCaseData.map(item => {
                const menuItem = this.menuItemsForComparison.find(mi => mi.caseData && mi.caseData.id === item.id);
                return menuItem ? menuItem.fullName : item.caseName;
              }),
              top: 30
            },
            grid: {
    ...
    exportComparison() {
      ...
      try {
        ...
        this.comparisonForm.selectedCases.forEach(caseId => {
          const menuItem = this.menuItemsForComparison.find(item => item.caseId === caseId);
          if (menuItem && menuItem.caseData) {
            selectedCases.push(menuItem.caseData);
          }
        });
        
        // 添加案例名称作为标题
        selectedCases.forEach(caseItem => {
          const menuItem = this.menuItemsForComparison.find(item => item.caseData && item.caseData.id === caseItem.id);
          csvContent += `,${menuItem ? menuItem.fullName : caseItem.caseName}`;
        });
        csvContent += '\n';
        ...
        timeLabels.forEach((timeLabel, timeIndex) => {
          ...
          selectedCases.forEach(caseItem => {
            ...
            // 根据案例类型和对比维度生成数据
            const isBaseline = caseItem.caseName && caseItem.caseName.includes('baseline');
            
            if (this.comparisonForm.dimension === 'potential') {
              // 潜在供给 - 基准情景和优化情景有不同的数据
              if (isBaseline) {
                const baseData = [65, 70, 76, 82, 90, 100, 95, 88, 80, 75, 70, 66];
                value = this.comparisonForm.timeScale === 'month' 
                  ? baseData[timeIndex % 12]
                  : [72, 78, 85, 90, 95][timeIndex % 5];
              } else {
                const optData = [70, 76, 82, 88, 95, 105, 100, 92, 85, 80, 75, 71];
                value = this.comparisonForm.timeScale === 'month'
                  ? optData[timeIndex % 12]
                  : [78, 84, 90, 96, 102][timeIndex % 5];
              }
            } else if (this.comparisonForm.dimension === 'actual') {
              // 实际利用 - 比潜在供给略低
              if (isBaseline) {
                const baseData = [52, 56, 61, 66, 72, 80, 76, 70, 64, 60, 56, 53];
                value = this.comparisonForm.timeScale === 'month'
                  ? baseData[timeIndex % 12]
                  : [58, 62, 68, 72, 76][timeIndex % 5];
              } else {
                const optData = [56, 61, 66, 70, 76, 84, 80, 74, 68, 64, 60, 57];
                value = this.comparisonForm.timeScale === 'month'
                  ? optData[timeIndex % 12]
                  : [62, 67, 72, 77, 82][timeIndex % 5];
              }
            } else {
              // 供需平衡 - 正负值
              if (isBaseline) {
                const baseData = [-1, 2, 5, 9, 14, 20, 17, 12, 8, 5, 2, -1];
                value = this.comparisonForm.timeScale === 'month'
                  ? baseData[timeIndex % 12]
                  : [3, 7, 11, 14, 17][timeIndex % 5];
              } else {
                const optData = [2, 6, 10, 13, 17, 23, 20, 15, 11, 8, 5, 2];
                value = this.comparisonForm.timeScale === 'month'
                  ? optData[timeIndex % 12]
                  : [7, 11, 15, 18, 22][timeIndex % 5];
              }
            }
    ...
    // 添加统一的数据刷新方法
    refreshData(regionServiceId, forceRefresh = false) {
      // 如果参数未变且不是强制刷新，则不重复获取数据
      if (!forceRefresh && regionServiceId === this.regionServiceId) {
        return;
      }
  ...
  watch: {
    ...
    // 添加对region_service_id的监听
    '$route.query.region_service_id': {
      handler(newVal) {
        if (newVal && newVal !== this.regionServiceId) {
    ...
    // 添加对regionServiceId的监听（兼容驼峰命名）
    '$route.query.regionServiceId': {
      handler(newVal) {
        if (newVal && newVal !== this.regionServiceId) {
          this.refreshData(newVal);
        }
